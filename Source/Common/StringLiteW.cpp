...
CNktStringW::CNktStringW() : CNktMemMgrObj()
{
  szStrW = NULL;
  nSize = nLen = 0;
  return;
...
BOOL CNktStringW::ConcatN(__in_nz_opt LPCWSTR szSrcW, __in SIZE_T nSrcLen)
{
  ...
  if (EnsureBuffer(nLen+nSrcLen) == FALSE)
    return FALSE;
  nktMemCopy(szStrW+nLen, szSrcW, nSrcLen*sizeof(WCHAR));
  nLen += nSrcLen;
  szStrW[nLen] = 0;
  return TRUE;
}
...
BOOL CNktStringW::AppendFormatV(__in LPCSTR szFormatA, __in va_list argptr)
{
  ...
  if (nChars > 0)
  {
    ...
    {
      szTempA = (LPSTR)nktMemMalloc((size_t)(nChars+1) * sizeof(CHAR));
      if (szTempA == NULL)
        return FALSE;
    }
    else
...
BOOL CNktStringW::InsertN(__in_nz_opt LPCWSTR szSrcW, __in SIZE_T nInsertPosition, __in SIZE_T nSrcLen)
{
  ...
  nktMemMove(szStrW+nInsertPosition+nSrcLen, szStrW+nInsertPosition, (nLen-nInsertPosition)*sizeof(WCHAR));
  nktMemCopy(szStrW+nInsertPosition, szSrcW, nSrcLen*sizeof(WCHAR));
  nLen += nSrcLen;
  szStrW[nLen] = 0;
  return TRUE;
}
...
VOID CNktStringW::Delete(__in SIZE_T nStartChar, __in SIZE_T nChars)
{
  ...
  if (nLen > 0)
  {
    ...
    s = szStrW + nStartChar;
    nLen -= nChars;
    nktMemMove(s, s+nChars, k*sizeof(WCHAR));
    szStrW[nLen] = 0;
  }
  return;
}
...
BOOL CNktStringW::EnsureBuffer(__in SIZE_T nChars)
{
  ...
  if (nChars >= nSize)
  {
    ...
    szNewStrW = (LPWSTR)nktMemMalloc(nChars*sizeof(WCHAR));
    if (szNewStrW == NULL)
      return FALSE;
    nktMemCopy(szNewStrW, szStrW, nLen*sizeof(WCHAR));
    szNewStrW[nLen] = 0;
    if (szStrW != NULL)
      nktMemFree(szStrW);
    szStrW = szNewStrW;
...
LPWSTR CNktStringW::Duplicate(__in LPWSTR szStrW, __in SIZE_T nLen)
{
  LPWSTR szStrCopyW;

  if (szStrW == NULL && nLen > 0)
    return NULL;
  if (nLen == (SIZE_T)-1)
    nLen = wcslen(szStrW);
  szStrCopyW = (LPWSTR)nktMemMalloc((nLen+1) * sizeof(WCHAR));
  if (szStrCopyW == NULL)
    return NULL;
  nktMemCopy(szStrCopyW, szStrW, nLen*sizeof(WCHAR));
  szStrCopyW[nLen] = 0;
  return szStrCopyW;
}
